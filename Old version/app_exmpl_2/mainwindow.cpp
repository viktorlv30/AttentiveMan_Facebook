...
void MainWindow::on_getRequest_clicked()
{
     ...
     */
        ...
        QString user_id = "1707734546174172";
        QString access_token = "EAACEdEose0cBACls5wXDpEPtZCXWcRBBcgxD9Q5UzsFdiU5jQtxwjTI7FEBsel31XlVY4DDIooCZAD1YH2pPfsjD7QlUQbiZA2FiU3WSwd3bcj3Q0jZBtZBCtX19xOnnhmwXsW9G7vGv2zZCldZBgeChaxaEYwQwBZCd92kILdVpsQZDZD";


        ui->lineToken->setText(access_token);
        ui->apiID->setText(user_id);

        access_token = ui->lineToken->text();
        user_id = ui->apiID->text();
...
//    ui->outJson->appendPlainText(strReplay);
    ...
    if(jsonResponse.isObject())
    {
        ...
        if( qm1.value("next").toString() != NULL )
        {
            ...
            int iNumber = propertyValues.count();
//            ui->countPosts->setText(QString::number(iNumber));
            ui->lcdNumber->display(propertyValues.count());
        }
    }
...
{
    mDialog.setValueStr("TEST");
    mDialog.exec();
    ui->outJson->appendPlainText(mDialog.getMyLineEdit());
}
